...
describe("Testing the products endpoints", () => {
  ...
  it("retrieving the /products/:id endpoint with a non existing id expect requests to be 404", async () => {
    const response = await request.get("/products/518a8715b0e660665c981723");
    expect(response.status).toBe(404);
  });

  it("deleting the /products/:id endpoint: expect successful 204 response code", async () => {
  ...
  it("should test that the GET /products endpoint is returning a list of products", async () => {
    ...
    expect(
      response.body.some(
        (p) => p._id.toString() === productResponse.body.product._id
      )
    ).toBe(true);
  });
